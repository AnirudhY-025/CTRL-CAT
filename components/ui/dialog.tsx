...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed right-0 top-0 z-50 flex h-full w-full max-w-xl flex-col border-l border-border bg-card shadow-[0_0_40px_rgba(35,34,52,0.16)] focus:outline-none sm:w-[520px]",
        className,
      )}
      {...props}
    >
...
function DialogHeader({
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-1 border-b border-border px-6 py-6 pr-14",
        className,
      )}
      {...props}
    />

        ...
        <main className="mx-auto max-w-[1500px] px-5 py-7 sm:px-8 sm:py-9 lg:px-10">
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
            <Card className="overflow-hidden">
              ...
              <div className="border-t border-border/70 px-5 py-3.5 text-xs text-muted-foreground">
                Showing{" "}
                  ...
                  <div className="space-y-1">
                    {localActivities.map((item, index) => (
                      <ActivityItem
                        key={item.id}
                        item={item}
                        isLast={index === localActivities.length - 1}
                        highlighted={selectedActivityId === item.id}
                      />
                    ))}
                  </div>
                ) : (

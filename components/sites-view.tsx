...
const MapCanvas = dynamic(() => import("./map/map-canvas"), {
  ssr: false,
  loading: () => (
...
const MAP_SITES = [
  {
    id: "S_001",
    name: "Central Rental Yard",
    code: "S_001",
    lat: 12.9716,
    lng: 77.5946,
    type: "yard",
    customer: "CAT Rental HQ",
  },
  {
    id: "S_003",
    name: "Riverbend Materials",
    code: "S_003",
    lat: 13.0123,
    lng: 77.6234,
    type: "customer",
    customer: "Sobha Constructions",
  },
  {
    id: "S_002",
    name: "Northline Expansion",
    code: "S_002",
    lat: 13.0512,
    lng: 77.5512,
    type: "customer",
...
function getEquipmentCoords(
  ...
  const site =
    MAP_SITES.find(
      (s) =>
        s.name.toLowerCase().includes(asset.site.toLowerCase()) ||
        s.code === asset.siteId,
    ) || MAP_SITES[0];
...
export function SitesView({
  ...
  const [transitList, setTransitList] = React.useState<TransitItem[]>(
    INITIAL_TRANSIT_ITEMS,
  );
  const [isPlaying, setIsPlaying] = React.useState(true);
  const [speedMultiplier, setSpeedMultiplier] = React.useState<number>(1);
  const [mapStyle, setMapStyle] = React.useState<
    "google_standard" | "google_satellite"
  >("google_standard");
  const [filterMode, setFilterMode] = React.useState<
    "all" | "in_transit" | "job_sites" | "yard"
  >("all");
  const [searchQuery, setSearchQuery] = React.useState("");

  const [selectedAsset, setSelectedAsset] = React.useState<Asset | null>(null);
  const [selectedTransitId, setSelectedTransitId] = React.useState<
    string | null
  >(null);
  const selectedTransit =
    transitList.find((t) => t.id === selectedTransitId) || null;
  const [selectedSite, setSelectedSite] = React.useState<
    (typeof MAP_SITES)[0] | null
  >(null);
    ...
    const interval = setInterval(() => {
      setTransitList((prev) =>
        prev.map((item) => {
          ...
          const newLat =
            item.startLat + (item.targetLat - item.startLat) * frac;
          const newLng =
            item.startLng + (item.targetLng - item.startLng) * frac;

          const remainingKm = item.totalDistanceKm * (1 - frac);
          const currentSpeed = Math.round(
            48 + Math.sin(Date.now() / 800 + Number(item.id.slice(-1))) * 6,
          );
          const newEta = Math.max(
            0,
            Math.round((remainingKm / Math.max(10, currentSpeed)) * 60),
          );
  ...
  async function handleCallDriver(item: TransitItem) {
    setCallStatus(
      `Dialing hauler driver ${item.driverName} (${item.driverPhone})…`,
    );
    try {
      ...
      setCallStatus(
        res.success
          ? `✓ Dispatch alert connected to ${item.driverName}`
          : `✗ Call failed`,
      );
    } catch {
  ...
  const filteredAssets = assets.filter((asset) => {
    const isTransit = transitList.some(
      (t) => t.assetId === asset.id && t.status !== "delivered",
    );
    const matchesSearch =
    ...
    if (filterMode === "job_sites")
      return !isTransit && asset.site !== "Central Rental Yard";
    if (filterMode === "yard")
      return !isTransit && asset.site === "Central Rental Yard";
    return true;
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div className="flex flex-wrap items-center gap-2">
          {[
            ...
            {
              id: "in_transit",
              label: `In Transit (${inTransitAssets.length})`,
            },
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.5fr)_1fr]">
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      const assetObj =
                        assets.find((a) => a.id === item.assetId) || null;
                    ...
                    <div className="mt-3 space-y-1">
                      <div className="flex justify-between text-[11px] font-semibold text-muted-foreground">
                        ...
                        <span className="text-foreground font-bold">
                          {item.progress}% ({item.speedKmh} km/h)
                        </span>
